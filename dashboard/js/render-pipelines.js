...
function openPipelineDetail(id) {
  ...
  var triggerDetail = escHtml(_getPipelineTriggerLabel(p)) + (p.trigger?.cron ? ' <span style="opacity:0.6">(' + escHtml(Intl.DateTimeFormat().resolvedOptions().timeZone) + ')</span>' : '');
  html += '<div style="display:flex;justify-content:space-between;align-items:center">' +
    '<span style="font-size:10px;color:var(--muted)">' + triggerDetail + ' · ' + escHtml(_getPipelineStageLabel(p)) + '</span>' +
